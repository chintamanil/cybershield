...
class OpenSearchVectorStore(BaseVectorStore):
    ...
    def __init__(
        self,
        collection_name: str = "cybersecurity_attacks",
        host: str = None,
        port: int = 443,
    ):
        ...
        self.port = port
        self.client = None
        self.index_name = collection_name.lower().replace(
            "_", "-"
    ...
    def _create_aws_client(self) -> OpenSearch:
        ...
        try:
            ...
            logger.error(
                "AWS authentication requires opensearch-py with AWSV4SignerAuth"
            )
            raise
    ...
    async def search_by_ip(
        ...
        try:
            ...
            logger.debug(
                "OpenSearch IP search completed",
                ip=ip_address,
    ...
    async def search_similar_attacks(
        ...
        try:
            ...
            logger.debug(
                "OpenSearch text similarity search completed",
                query=query_text[:50],
                results_count=len(results),
            )
            ...
            logger.error(
                "OpenSearch similarity search failed", query=query_text, error=str(e)
            )
            return []
    ...
    async def search_by_vector(
        ...
        try:
            ...
            logger.debug(
                "OpenSearch vector search completed",
    ...
    async def get_attack_stats(self, ip_address: str) -> dict[str, Any]:
        ...
        try:
            ...
            search_body = {
                ...
                "aggs": {
                    "attack_types": {"terms": {"field": "attack_type", "size": 50}},
                    "severity_levels": {
                        "terms": {"field": "severity_level", "size": 10}
                    },
                    "most_recent": {"max": {"field": "timestamp"}},
                    "avg_anomaly_score": {"avg": {"field": "anomaly_score"}},
                    "is_source": {"filter": {"term": {"source_ip": ip_address}}},
                    "is_destination": {"filter": {"term": {"dest_ip": ip_address}}},
                },
            }
            ...
            high_severity = attack_types.get("Critical", 0) + attack_types.get(
                "High", 0
            )
            avg_anomaly = aggs["avg_anomaly_score"]["value"] or 0
            risk_score = min(
                100, total_attacks * 5 + high_severity * 20 + avg_anomaly * 10
            )
            ...
            return {
                ...
                "most_recent": (
                    aggs["most_recent"]["value_as_string"]
                    if aggs["most_recent"]["value"]
                    else None
                ),
    ...
    async def index_document(
        self, document: dict[str, Any], doc_id: str = None
    ) -> bool:
    ...
    async def bulk_index(self, documents: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                ...
                doc_id = (
                    doc.get("id")
                    or f"doc_{i}_{hashlib.md5(json.dumps(doc, sort_keys=True).encode()).hexdigest()[:8]}"
                )

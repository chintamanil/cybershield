...
def make_api_request(
    ...
    try:
        ...
        request_kwargs = {
            "timeout": 30,
            "verify": (
                False if USE_AWS_BACKEND else True
        ...
        if USE_AWS_BACKEND:
            st.error(
                f"❌ Cannot connect to AWS backend after {elapsed_time:.2f}s. Please check: {AWS_BACKEND_URL}"
            )
        else:
            st.error(
                f"❌ Cannot connect to local backend after {elapsed_time:.2f}s. Please ensure the server is running on {FASTAPI_URL}"
            )
        return None
        ...
        if st.button("🔍 Check System Status", use_container_width=True):
            ...
            if status:
                ...
                if "agents" in status:
                    ...
                    vectorstore_available = supervisor.get(
        ...
        if st.button("Check IP", use_container_width=True) and ip_input:
            with st.spinner("Checking IP reputation..."):
                result = make_api_request(
                    "/tools/abuseipdb/check", "POST", {"ip_address": ip_input}
                )
                if result:
                    st.json(result)
        ...
        if st.button("Analyze Domain", use_container_width=True) and domain_input:
            with st.spinner("Analyzing domain..."):
                result = make_api_request(
                    ...
                    "POST",
                    {"resource": domain_input, "resource_type": "domain"},
                )
                if result:
                    st.json(result)
                    ...
                    if inputs:
                        with st.expander("Preview (first 5 entries)"):
                            for i, inp in enumerate(inputs[:5], 1):
                                st.write(
                                    f"{i}. {inp[:100]}{'...' if len(inp) > 100 else ''}"
                                )
                ...
                if result:
                    ...
                    if "results" in result:
                        ...
                        for i, res in enumerate(results_data, 1):
                            pii_detected = res.get("pii_analysis", {}).get(
                                "pii_detected", False
                            )
                            ioc_count = res.get("ioc_analysis", {}).get("ioc_count", 0)
                            threat_level = res.get("threat_analysis", {}).get(
                                "overall_risk", "low"
                            )
    ...
    with tools_tab:
        ...
        with tool_cols[1]:
            ...
            pattern_type = st.selectbox(
                "Pattern type:", ["ip", "domain", "hash", "url"]
            )

            if (
                st.button("Validate Pattern", use_container_width=True)
                and validation_text
            ):
        ...
        with vector_cols[0]:
            ...
            vector_ip = st.text_input(
                "IP Address for Vector Search:", placeholder="192.168.1.1"
            )
            vector_limit = st.slider("Max Results:", 1, 50, 10)

            if (
                st.button("Search Vector Database", use_container_width=True)
                and vector_ip
            ):
                with st.spinner("Searching historical attack data..."):
                    ...
                    st.write(
                        "This would search the Milvus vector database for historical attacks involving:",
                        vector_ip,
                    )
        ...
        with vector_cols[1]:
            ...
            if (
                st.button("Batch Vector Search", use_container_width=True)
                and vector_ips
            ):
                ...
                with st.spinner(f"Searching {len(ip_list)} IPs in vector database..."):
                    ...
                    st.write(
                        f"This would search for historical attacks involving {len(ip_list)} IPs:",
                        ip_list,
                    )
            ...
            with st.spinner("Checking vector database status..."):
                ...
                if status and "system_info" in status:
                    ...
                    with col1:
                        ...
                        st.write(
                            f"**Vector Store:** {status_icon} {'Available' if vectorstore_available else 'Unavailable'}"
                        )
                    ...
                    with col2:
                        ...
                        st.write(
                            f"**ReAct Workflow:** {react_icon} {'Enabled' if react_enabled else 'Disabled'}"
                        )

                    if vectorstore_available:
                        st.success(
                            "🗃️ Vector database is connected and ready for historical attack searches"
                        )
                    else:
                        st.warning(
                            "⚠️ Vector database not available - historical search functionality limited"
                        )

...
class APIClient:
    ...
    def analyze_text(
        self, text: str, use_react_workflow: bool = True, include_vision: bool = False
    ) -> Optional[Dict]:
    ...
    def analyze_with_image(
        ...
        image_data: bytes,
        filename: str,
        use_react_workflow: bool = True,
    ) -> Optional[Dict]:
        """Analyze text with image"""
        files = {"image": (filename, image_data)}
        data = {"text": text, "use_react_workflow": use_react_workflow}
        return self._make_request("/analyze-with-image", "POST", data=data, files=files)

    def batch_analyze(
        self, inputs: List[str], use_react_workflow: bool = True
    ) -> Optional[Dict]:
        """Analyze multiple text inputs"""
        return self._make_request(
            "/batch-analyze",
            "POST",
            {"inputs": inputs, "use_react_workflow": use_react_workflow},
        )
    ...
    def check_ip_abuseipdb(self, ip_address: str) -> Optional[Dict]:
        ...
        return self._make_request(
            "/tools/abuseipdb/check", "POST", {"ip_address": ip_address}
        )

    def lookup_ip_shodan(self, ip_address: str) -> Optional[Dict]:
        """Lookup IP with Shodan"""
        return self._make_request(
            "/tools/shodan/lookup", "POST", {"ip_address": ip_address}
        )

    def lookup_virustotal(
        self, resource: str, resource_type: str = "ip"
    ) -> Optional[Dict]:
    ...
    def validate_pattern(self, text: str, pattern_type: str) -> Optional[Dict]:
        ...
        return self._make_request(
            ...
            "POST",
            {"text": text, "pattern_type": pattern_type},
        )

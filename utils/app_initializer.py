...
class ApplicationInitializer:
    ...
    async def initialize_services(self) -> Dict[str, Any]:
        ...
        try:
            services.redis.ping()
            initialization_results['redis'] = {'status': 'success', 'host': config.redis.host}
        ...
        try:
            ...
            cursor.close()
            initialization_results['postgres'] = {'status': 'success', 'version': version}
        ...
        try:
            ...
            logger.info(f"Vector store ({config.vector_store.provider}) initialized successfully")
        except Exception as e:
            initialization_results['vector_store'] = {'status': 'error', 'error': str(e)}
    ...
    def get_environment_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'detection_method': 'automatic' if config.detector._is_aws_environment() else 'manual',
            'services': {
                'database': {
                    'provider': 'RDS' if config.detector.is_aws() else 'Local PostgreSQL',
                    'host': config.database.host,
                    'ssl_enabled': config.database.ssl_mode == 'require'
                },
                'cache': {
                    'provider': 'ElastiCache' if config.detector.is_aws() else 'Local Redis',

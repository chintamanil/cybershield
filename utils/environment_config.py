...
class EnvironmentDetector:
    ...
    def _detect_environment(self) -> str:
        ...
        if env in ["local", "development", "dev"]:
            return "local"
        elif env in ["aws", "cloud", "production"]:
            return "aws"
    ...
    def _is_aws_environment(self) -> bool:
        ...
        # Only check for actual AWS runtime environments, not local AWS CLI config
        aws_indicators = [
            os.getenv("AWS_EXECUTION_ENV"),  # ECS/Lambda
            os.getenv("AWS_LAMBDA_FUNCTION_NAME"),  # Lambda
            os.getenv("ECS_CONTAINER_METADATA_URI"),  # ECS
            # Note: AWS_REGION removed - developers often have this set locally for AWS CLI
        ]
...
class EnvironmentConfig:
    ...
    def _load_aws_config(self):
        ...
        self.database = DatabaseConfig(
            host=os.getenv(
                "RDS_ENDPOINT",
                "cybershield-db.cluster-xxxxx.us-east-1.rds.amazonaws.com",
            ),
        ...
        self.redis = RedisConfig(
            host=os.getenv(
                "ELASTICACHE_ENDPOINT", "cybershield-cache.xxxxx.cache.amazonaws.com"
            ),
        ...
        self.vector_store = VectorStoreConfig(
            ...
            host=os.getenv(
                "OPENSEARCH_ENDPOINT",
                "search-cybershield-xxxxx.us-east-1.es.amazonaws.com",
            ),
            port=443,
            ssl=True,
            auth_required=True,

...
def make_request(text: str, session_id: str) -> dict:
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/analyze",
            json={"text": text, "session_id": session_id, "use_react_workflow": True},
            timeout=30,
        )
...
def demo_scenario_1():
    ...
    response1 = make_request(
        "Suspicious login attempt from IP 192.168.1.100", session_id
    )
    ...
    response2 = make_request(
        "The same IP is now scanning ports 22, 23, 3389", session_id
    )
...
def demo_scenario_2():
    ...
    text1 = (
        "Security incident: Traffic from 203.0.113.50 to malicious-c2.example.com. "
...
def main():
    ...
    console.print(
        "[bold green]CyberShield Context Preservation Demonstration[/bold green]"
    )
    ...
    console.print("[dim]Make sure the CyberShield backend is running![/dim]\n")

...
class TestSupervisorAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        with (
            patch("agents.supervisor.PIIAgent") as mock_pii,
    ...
    async def test_generate_recommendations_high_risk(self):
        ...
        analysis_results = {
            ...
            "pii_analysis": {"mapping": {"[PII_SSN_001]": "123-45-6789"}},
            "vision_analysis": {
        ...
        self.assertTrue(
            any(keyword in rec_text for keyword in ["isolate", "block", "immediate"])
        )
    ...
    async def test_concurrent_agent_operations(self):
        ...
        )  # Should be much faster than 3x0.1 = 0.3s
        self.assertEqual(len(results), 3)
            ...
            mock_threat_agent.evaluate.return_value = [
                {
                    "ioc": "203.0.113.1",
                    "ioc_type": "ip",
                    "summary": {"risk_score": 85, "is_malicious": True},
                },
                {
                    "ioc": "malicious.example.com",
                    "ioc_type": "domain",
                    "summary": {"risk_score": 75, "is_malicious": True},
                },
            ]

            mock_pii_agent = AsyncMock()
            mock_pii_agent.mask_pii.return_value = (
                "Masked security log",
                {"[MASK_0]": {"original": "admin@company.com", "type": "email"}},
            )

...
class TestPIIAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_unmask_text_success(self):
        ...
        # Setup mock mappings
        mock_mappings = {"[MASK_0]": "john@example.com", "[MASK_1]": "123-45-6789"}
    ...
    async def test_unmask_text_no_session(self):
        ...
        self.mock_store.get_session_mappings.return_value = {
            "[MASK_0]": "test@example.com"
        }
    ...
    async def test_pii_patterns_coverage(self):
        ...
        for text, pii_type in test_cases:
            ...
            self.assertTrue(
                found_correct_type, f"Expected {pii_type} pattern not found"
            )
...
class TestPIIAgentIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_workflow(self):
        ...
        self.mock_store.get_session_mappings.return_value = {
            token: data["original"] for token, data in mapping.items()
        }

...
async def test_ioc_storage():
    ...
    success = await session_storage.store_iocs(
        session_id=session_id, iocs=iocs_1, merge=True
    )
    ...
    print(
        f"Last IP: {retrieved_1.get('ips', [])[-1] if retrieved_1 and retrieved_1.get('ips') else 'NONE'}"
    )

    # Test 2: Store second set of IOCs (merge with existing)
    print("\n--- Test 2: Store second IOCs (merge mode) ---")
    iocs_2 = {"ips": ["198.51.100.25"], "domains": [], "hashes": []}

    success = await session_storage.store_iocs(
        session_id=session_id, iocs=iocs_2, merge=True
    )
    print(f"Storage result: {'✓ Success' if success else '✗ Failed'}")
    ...
    if context and "ip" in needs_context and context.get("iocs", {}).get("ips"):
        ...
        print("Expected IP: 198.51.100.25")
        print(f"Actual IP: {context_used.get('ip', 'NONE')}")
        print(
            f"Match: {'✓ CORRECT' if context_used.get('ip') == '198.51.100.25' else '✗ WRONG'}"
        )
    else:
        ...
        print(
            f"  - Has IPs: {bool(context.get('iocs', {}).get('ips') if context else False)}"
        )

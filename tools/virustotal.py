...
class VirusTotalClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            logger.warning(
                "VirusTotal API key not provided - functionality will be limited"
            )
    ...
    async def lookup_ip(self, ip_address: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "malicious_count": attributes.get("last_analysis_stats", {}).get(
                "malicious", 0
            ),
            "suspicious_count": attributes.get("last_analysis_stats", {}).get(
                "suspicious", 0
            ),
            "harmless_count": attributes.get("last_analysis_stats", {}).get(
                "harmless", 0
            ),
